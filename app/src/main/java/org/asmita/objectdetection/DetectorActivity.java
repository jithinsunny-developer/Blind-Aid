...
public class DetectorActivity extends CameraActivity implements OnImageAvailableListener
{
  ...
  private final String ERROR_COULDNT_READ_LEFT = "nothing on your left!";
  private final String ERROR_COULDNT_READ_RIGHT = "nothing on your right!";
  private final String ERROR_COULDNT_READ_FRONT = "nothing in front of you!";

  private final String REGEX_WHATS_ON_MY_LEFT= "what('s| is) on my left";
  private final String REGEX_WHATS_ON_MY_RIGHT= "what('s| is) on my right";
  private final String REGEX_WHATS_IN_FRONT= "what('s| is) in front of me";
...
//    String leftSentence = formSentenceleft(objectsOnLeft);
//    String rightSentence = formSentenceright(objectsOnRight);
//    String centreSentence = formSentencecentre(objectsInFront);
  ...
  private void speakRecognizedTextLeft()
  {
    String objectsOnLeft = formPartialSentence(objectsToSpeak.get(POSITION_LEFT));
   //Log.d("gonna speak", objectsOnLeft);


    if(objectsOnLeft.isEmpty())
    {
      tts.speak(ERROR_COULDNT_READ_LEFT, TextToSpeech.QUEUE_ADD, null);
    }
    else
      {
      tts.speak(objectsOnLeft, TextToSpeech.QUEUE_ADD, null);
    }
  ...
  private void speakRecognizedTextRight()
  {
    String objectsOnRight = formPartialSentence(objectsToSpeak.get(POSITION_RIGHT));
    if(objectsOnRight.isEmpty())
    {
      tts.speak(ERROR_COULDNT_READ_RIGHT, TextToSpeech.QUEUE_ADD, null);
    }
    else
    {
      tts.speak(objectsOnRight, TextToSpeech.QUEUE_ADD, null);
    }
  }

  private void speakRecognizedTextFront()
  {
    String objectsInFront = formPartialSentence(objectsToSpeak.get(POSITION_FRONT));
    if(objectsInFront.isEmpty())
    {
      tts.speak(ERROR_COULDNT_READ_FRONT, TextToSpeech.QUEUE_ADD, null);
    } else
    {
      tts.speak(objectsInFront, TextToSpeech.QUEUE_ADD, null);
    }
  }
            ...
            new SpeechRecognitionListener(this, new SpeechRecognitionListener.OnSpeechRecognitionResult() {
              ...
              public void onSuccess(ArrayList<String> matches)
              {
                for(String match: matches)
                {
                  ...
                  else if (matches(REGEX_WHATS_ON_MY_LEFT, match))
                  {
                    //lastSpokenTimeStamp = 0;
                    //shouldGuide = true;
                    speakRecognizedTextLeft();
                    break;


                  }
                  else if (matches(REGEX_WHATS_ON_MY_RIGHT, match))
                  {
                    //lastSpokenTimeStamp = 0;
                    //shouldGuide = true;
                    speakRecognizedTextRight();
                    break;

                  }

                  else if(matches(REGEX_WHATS_IN_FRONT, match))
                  {
                    //lastSpokenTimeStamp=0;
                    //shouldGuide= true;
                    speakRecognizedTextFront();
                    break;
                  }
